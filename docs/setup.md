...
Short-meeting paid inference and the deployed Vercel worker are verified; see [Deployment](deployment.md). Google OAuth is configured and a real browser recording is verified. The public privacy policy is deployed. Self-service account deletion, operational alerting, and the long-recording deployment benchmark remain launch work. Existing requirements documents describe that broader launch target.
